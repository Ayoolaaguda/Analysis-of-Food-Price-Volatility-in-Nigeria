...
Select ID, Commodity, Round(Avg(Average_Price),2) as AvgPrice
From SFI
Left join Month_Hierarchy as MH
on SFI.Month= MH.Month_Name
...
Select Commodity, Month, Season, Avg(Average_Price) as AvgPrice
From SFI
Left join Month_Hierarchy as MH
on SFI.Month= MH.Month_Name
Group by Commodity, Season, Month
Order by Commodity, AvgPrice desc


--Seasonal Trends For Create View
Create view SeasonalPrice
as
With SFI as (Select*
From SFI_2022
Union all
...
Select Distinct(Commodity), ID,Month,Season, Round(Avg(Average_Price)
Over (Partition by commodity,Month),2) as AvgPrice
From SFI
Left join Month_Hierarchy as MH
on SFI.Month= MH.Month_Name
Group by Commodity, ID, Month, Season,Average_Price
--Order by Commodity, AvgPrice desc
go
...
--Where Year in (2022,2023) --Price Votality for 2022 and 2023
Group by Commodity
...
Select Commodity, ID, Month,Year,Average_Price, Round(Avg(Average_Price)
Over (Partition by commodity Order by Year rows between 2 preceding and current row),2) as Three_Months_Moving_Average
From SFI
Left join Month_Hierarchy as MH
on SFI.Month= MH.Month_Name
Group by Commodity,ID, Month,Year,Average_Price
